...
def step(conn, move=0, strafe=0, attack=0, jump=0, sprint=0, reset=0):
    n = conn.hello.arenas
    z = np.zeros(n, np.float32)
    conn.send_actions(z, z, np.full(n, attack, np.uint8),
...
def taken_per_episode(conn, opponent, juggle, episodes=6):
    ...
    while done_eps < episodes:
        if juggle:
            ...
            attack = (charge >= 0.999).astype(np.uint8)
            obs = step(conn, move=1, sprint=1, attack=int(attack[0]))
        else:
